...
COMMAND="sudo docker build $BUILD_CACHE --rm=true -t $IMAGE_NAME:$TAG --build-arg HTTPS_COMMENT=$HTTPS_COMMENT --build-arg USER_ID=$USER_ID --build-arg USER_PASS=$USER_PASS --build-arg USER_UID=$USER_UID --build-arg USER_GID=$USER_GID . 2>&1 | tee $(date +"%Y%m%d-%H%M%S").log"
