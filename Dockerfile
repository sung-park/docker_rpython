...
RUN /bin/bash ~/downloads/Anaconda2-4.0.0-Linux-x86_64.sh -b
...
matplotlib notebook numpy pandas pip pymc pymongo pytables pyzmq requests \
scipy scikit-image scikit-learn seaborn setuptools supervisor \
virtualenv \
&& echo
...
RUN pip install bash_kernel filterpy fysom JPype1 keras konlpy nlpy pudb rpy2
...
RUN mkdir -p /home/$USER_ID/data/tsa
RUN unzip /home/$USER_ID/Datasets.zip -d /home/$USER_ID/data/tsa
...
RUN python -W ignore -c "from sklearn.datasets import *; fetch_rcv1()"
